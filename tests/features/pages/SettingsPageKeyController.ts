...
export default class SettingsPageKeyController {
    ...
    public async addKey(keyPair: {
        ...
        assertTruthy(page);
        const typeInputsContainer = page.locator('*[data-testid="keypair-form-type"] .MuiInput-formControl').last();
        ...
        const publicKeyInputs = await this.queries.queryAllByLabelText(document, 'publicKeyMultibase *');
        ...
        const privateKeyInputs = await this.queries.queryAllByLabelText(document, 'privateKeyMultibase *');
    ...
    public async getKeys() {
        const document = await this.testingLibrary.getDocument();
        assertTruthy(document);
        const keyPairForms = await this.queries.getAllByTestId(document, 'keypair-form');
        const keys = await Promise.all(keyPairForms.map(async formEl => {
            const key = {
                ...
                publicKeyMultibase: await (await this.queries.getByLabelText(formEl, 'publicKeyMultibase *')).getAttribute('value'),
                privateKeyMultibase: await (await this.queries.getByLabelText(formEl, 'privateKeyMultibase *')).getAttribute('value'),
            };
            return key;
        }));
        return keys;
